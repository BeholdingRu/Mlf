...
create table if not exists public.profiles (
  ...
  annual_cycle_enabled boolean not null default false,
  last_bible_book_order smallint check (last_bible_book_order between 1 and 66),
...
-- Bible reading tree: one progress step equals 0.3%.
...
returns table (progress_steps integer, chapters_today integer, started_on date)
language plpgsql
security invoker
set search_path = public
as $$
...
begin
  ...
  if not found then
    return query
      select 0, count(*)::integer, null::date
      from public.bible_chapter_reads as reads
      where reads.user_id = current_user_id and reads.read_on = today;
    return;
  end if;
  ...
  if progress_row.last_processed_on < today - 1 then
    ...
    set progress_steps = next_progress, last_processed_on = today - 1, updated_at = now()
    where progress.user_id = current_user_id;
  end if;
  ...
  return query select next_progress, chapter_count, progress_row.started_on;
end;
$$;

create or replace function public.record_bible_chapter_read(p_book_order smallint, p_chapter smallint)
returns table (progress_steps integer, chapters_today integer, started_on date)
language plpgsql
security invoker
set search_path = public
...
begin
  if current_user_id is null then raise exception 'Authentication required'; end if;
  if p_book_order not between 1 and 66 or p_chapter < 1 then raise exception 'Invalid Bible chapter'; end if;
  today := public.bible_progress_local_date();

  insert into public.bible_tree_progress (user_id, progress_steps, started_on, last_processed_on)
  values (current_user_id, 0, today, today - 1)
  on conflict (user_id) do nothing;

  insert into public.bible_chapter_reads (user_id, read_on, book_order, chapter)
  values (current_user_id, today, p_book_order, p_chapter)
  on conflict (user_id, read_on, book_order, chapter) do nothing;

  return query select * from public.refresh_bible_tree_progress();
end;
$$;
